...
const createOrder = async (req, res) => {
  try {
    ...
    const { selectedAddress, paymentMethod, fromCart, products, razorpayPaymentId, originalAmount, discountedAmount, useWallet } = req.body;
    console.log(discountedAmount + " hi iam here");
    ...
    const deductAmount = originalAmount - discountedAmount;
    ...
    if (useWallet) {
      ...
      // Deduct the discounted amount from the wallet balance
      user.wallet.balance -= deductAmount;
      await user.save();
    }
    console.log(userId,orderProducts,originalAmount,paymentMethod,selectedAddress)
...
function calculateDeliveryDate() {
  const deliveryDate = moment().add(7, 'days'); // Use moment.js to calculate delivery date
  return deliveryDate;
}
...
const cancelOrder = async (req, res) => {
  try {
    ...
    console.log("this is cancel order");
